...
void DFRobot_LCD::init()
{
    ...
    i2c_config_t conf;
    conf.mode = I2C_MODE_MASTER; // Set as master
    ...
    conf.master.clk_speed = I2C_MASTER_FREQ_HZ; // Set clock speed
    conf.clk_flags = 0;
...
void DFRobot_LCD::begin(uint8_t cols, uint8_t lines, uint8_t dotsize) 
{
    ...
    vTaskDelay(5);  // wait more than 4.1ms
...
void DFRobot_LCD::send(uint8_t *data, uint8_t len) {
        ...
        i2c_master_write_byte(cmd, (_lcdAddr << 1) | I2C_MASTER_WRITE, true); // Address + write bit

        for(int i = 0; i < len; i++) {
            i2c_master_write_byte(cmd, data[i], true); // Write data byte
            vTaskDelay(pdMS_TO_TICKS(5)); // Delay for 5 ms
        }

        i2c_master_stop(cmd); // Stop I2C transmission
        i2c_master_cmd_begin(I2C_NUM_0, cmd, pdMS_TO_TICKS(1000)); // Execute the command
        i2c_cmd_link_delete(cmd); // Free command link
    }
...
void DFRobot_LCD::setReg(uint8_t addr, uint8_t data) {
        ...
        // Start I2C transmission
        i2c_master_start(cmd);
        i2c_master_write_byte(cmd, (_RGBAddr << 1) | I2C_MASTER_WRITE, true); // Address + write bit
        i2c_master_write_byte(cmd, addr, true); // Write register address
